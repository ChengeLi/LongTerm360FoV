...
if cfg.use_xyz:
    ...
    all_video_data = pickle.load(open('/scratch/wz1219/FoV/data/new_exp_1_xyz.p','rb'))
    data_dim = 3
#all_video_data = clip_xyz(all_video_data)
datadb = all_video_data.copy()


def reshape2second_stacks(per_video_db,collapse_user=False,stride = 1):
...
# Save model
model.save('fov_s2s_tanh_2layers.h5')
...
decoder_states_inputs = decoder1_states_inputs+ decoder2_states_inputs
...
pickle.dump(decoded_sentence_list,open('decoded_sentence_2LAYERS.p','wb'))
pickle.dump(gt_sentence_list,open('gt_sentence_list_2LAYERS.p','wb'))
