...
from code.dataLayer import DataLayer
import code.cost as costfunc
from code.config import cfg
from code.dataIO import clip_xyz
from code.utility import reshape2second_stacks,get_data,_create_one_hot
from code.utility import get_shuffle_index,shuffle_data,get_gt_target_xyz,get_gt_target_xyz_oth
from code.utility import slice_layer,rand_sample_ind,rand_sample
...
from code.utility import generate_fake_batch_numpy
from code.data_generator_for_heatmap import *
...
### ====================Graph def====================
if cfg.use_one_hot:
    input_shape1 = (batch_size,36,18,fps)
    input_shape2 = (batch_size,36,18,latent_dim*2)
    input_shape3 = (batch_size,36,18,latent_dim)
else:
...
if cfg.use_one_hot:
    ...
    pred_conv_lstm_conv1 = Conv2D(filters=1024, kernel_size=(kernel_size,kernel_size), padding='same',
        activation='relu', use_bias=True, kernel_initializer='glorot_uniform')
    pred_conv_lstm_conv2 = Conv2D(filters=fps, kernel_size=(kernel_size,kernel_size), padding='same',   # filters=fps
        activation='relu', use_bias=True, kernel_initializer='glorot_uniform')
else:
...
#weights = np.ones(30)
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])#costfunc.likelihood_loss())#'categorical_crossentropy')
...
if not cfg.use_one_hot:
    ...
    video_data_train = pickle.load(open('/scratch/wz1219/FoV/data/shanghai_dataset_xyz_train.p','rb'))    
    ...
    datadb = pickle.load(open('/scratch/wz1219/FoV/data/shanghaitech1/shanghaitech_heatmap.p','rb'))
    num_samples = 10000
    mygenerator = generator_train2(datadb)
    mygenerator_val = generator_train2(datadb,phase='val')
    '''
    data_format=2
    name = '_video_db_tar'
...
if 0:#cfg.sample_and_refeed or cfg.stateful_across_batch:
...
model.fit_generator(mygenerator, steps_per_epoch=num_samples/10,epochs=epochs,
...
#video_data_test = pickle.load(open('/scratch/wz1219/FoV/data/shanghai_dataset_xyz_test.p','rb'))
...
while len(gt_sentence_list)<2000:
    x, gt_temp = my_generator_test.next()
    deccoded_sentence_list_temp = model.predict_on_batch(x)
    decoded_sentence_list.append(decoded_sentence_list_temp)
    gt_sentence_list.append(gt_temp)
