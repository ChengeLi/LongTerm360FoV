"""
This is the social pooling mechanism. 
Modified from the original social LSTM
"""
...
from tf_normal_sampler import normal2d_sample,normal2d_log_pdf
    ...
    def _build_model(self, config):
        ...
        for t in range(config.obs_len):
            ...
            for ped_index in range(config.max_n_peds):
                ...
                x_pos_it = Lambda(lambda x_t: x_t[:, ped_index, 1:])(x_t)#social lstm only has xy
                # x_pos_it = Lambda(lambda x_t: x_t[:, ped_index, :])(x_t) #we have xyz
        ...
        for t in range(config.pred_len):
            ...
            grid_t = tf_grid_mask(x_pred_t, image_size,
